...
public:
	void Render() {
		...
		DrawText(program1, 1, "Legend of George", 0.5f, -0.25f);
...
public:
	void Render() {
		...
		if (gameState.success) {
			...
			modelMatrix = glm::scale(modelMatrix, glm::vec3(0.5f, 0.5f, 0.0f));
			program1.SetModelMatrix(modelMatrix);
			DrawText(program1, 1, "GEORGE WON!", 0.5f, -0.25f);
...
void Setup() {
	...
	displayWindow = SDL_CreateWindow("Legend of Zelda", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, resX, resY, SDL_WINDOW_OPENGL);
